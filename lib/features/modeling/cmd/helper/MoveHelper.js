 ...
 */
MoveHelper.prototype.moveClosure = function(closure, delta, newParent, isAttach) {
  ...
  // if it is one shape,isAttach === false and it has a host -> detach
  // multiple and isAttach
  ...
  forEach(allShapes, function(s) {
    ...
    // console.log('MoveHelper');
    // console.log(isAttach, isOneShapeMoving, s);
    ...
    if (isOneShapeMoving) {
      if (isAttach || s.host) {
        return modeling.attachShape(s, topLevel[s.id] && newParent, isAttach);
      }
    }
  });
